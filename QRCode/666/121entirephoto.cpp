...
#define tol_distance 80
using namespace std;
using namespace cv;

Mat gray_img, bw_img, gray, bw;
Mat src;
...
{
	Mat roi_img;
	const char* imagename = "1201.png";
	src = imread(imagename, 1);
	Mat hole = Mat::zeros(src.rows, src.cols, CV_8UC1);
	...
	*/
	if (points.size() >= 3)
	{
		...
		*/
		...
		for (it = group[score_index[3]].begin(); it != group[score_index[3]].end(); ++it)
		{
			qr_point4 += (*it);
		}
		qr_point4.x = qr_point4.x / score[3];
		qr_point4.y = qr_point4.y / score[3];
		...
		draw_crosshair(qr_point4, src, red);
...
void DrawPolygon(Mat img)
{
	...
	rookPoints[0][0] = card1;
	rookPoints[0][1] = card2;
	rookPoints[0][2] = card4;
	rookPoints[0][3] = card3;
