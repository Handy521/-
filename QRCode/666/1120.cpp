...
int _tmain()
{
	...
	//const char* imagename = "1120.png";
	//src = imread(imagename, 1);
	Mat src = imread("1120.png",1);
...
/*	//a = 1.5 * card1.y - 0.5*card2.y;
	//b = 2 * card2.y - card1.y;
	//c = card1.x - ((card3.x - card1.x) / );
	//d = card3.x + ((card3.x - card1.x) / 8);
	Rect rect(card1.x, card1.y, 200, 1500);
	//roi_img = src(Range(a, 623), Range(0, 2000));
	 src (rect).copyTo(roi_img);
		//Range(card1.x - ((card3.x - card1.x) / 8), card3.x + ((card3.x - card1.x) / 8)));
	namedWindow("cut image",0);
	imshow("cut image", roi_img);*/
	//imwrite("extractimg.png",roi_img);
	waitKey(0);
	return 0;
}
